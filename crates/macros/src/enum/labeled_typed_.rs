#[macro_export]
macro_rules! labeled_typed_enum {
    ($enum_identifier:ident, $variant:ty,  $label:expr,
     [ $( [$identifier:ident, $content:ty, $const_identifier:ident, $discriminant:expr, $description:expr, $acronym:expr] ),* $(,)? ]) => {
        ...
        impl $enum_identifier {
            pub fn from(discriminant: $variant) -> Self {
                match discriminant {
                    $($discriminant => Self::$identifier(<$content>::TODO),)*
                    _ => Self::TODO,
                }
            }

            pub fn to(&self) -> ($variant,$variant) {
                match *self {
                    $(Self::$identifier(c) => ($discriminant,c.to()),)*
                    _ => (<$variant>::MAX,<$variant>::MAX),
                }
            }
